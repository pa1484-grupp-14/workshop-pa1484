...
class LilyGo_Display {
 public:
  ...
  virtual void setAddrWindow(uint16_t xs, uint16_t ys, uint16_t xe,
                             uint16_t ye) = 0;
  virtual void pushColors(uint16_t* data, uint32_t len) = 0;
  virtual void pushColors(uint16_t x, uint16_t y, uint16_t width,
                          uint16_t height, uint16_t* data) = 0;
