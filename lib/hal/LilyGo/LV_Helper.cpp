...
static void mouse_read(lv_indev_t* indev, lv_indev_data_t* data) {
  ...
  if (xQueueReceive(params_copy.queue, &msg, pdTICKS_TO_MS(30)) == pdPASS) {
    if (msg.id == 'm') {
      ...
      data->state = (msg.left || msg.right) ? LV_INDEV_STATE_PRESSED
...
void beginLvglHelper(LilyGo_Display& board, bool debug) {
  ...
  if (board.needFullRefresh()) {
    lv_display_set_buffers(disp_drv, buf, buf1, lv_buffer_size,
                           LV_DISPLAY_RENDER_MODE_FULL);
  } else {
    lv_display_set_buffers(disp_drv, buf, buf1, lv_buffer_size,
                           LV_DISPLAY_RENDER_MODE_PARTIAL);
    lv_display_add_event_cb(disp_drv, lv_rounder_cb, LV_EVENT_INVALIDATE_AREA,
                            NULL);
  }
